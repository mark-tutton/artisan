...
# Make this optional for test builds - set fatal=False
...
copy_file(YOCTO_BIN + r'\yapi64.dll', TARGET + r'_internal\yoctopuce\cdll', fatal=False)
# YOCTO HACK END

# copy Snap7 lib
copy_file(SNAP7_BIN + r'\snap7.dll', TARGET + r'_internal', fatal=False)
...
# Get total size in bytes minus the vc_redist.x64.exe file (if it exists)
vc_redist_path = TARGET + 'vc_redist.x64.exe'
if os.path.exists(vc_redist_path):
    size_bytes = get_size(TARGET) - get_size(vc_redist_path)
else:
    size_bytes = get_size(TARGET)
