        ...
        try:
            ...
            if inventory_plugin and hasattr(inventory_plugin, "fetcher"):
                ...
                if inventory_fetcher:
                    ...
                    # Test connection to server (but don't create new connections)
                    is_connected = False
                    try:
                        # Only test if fetcher is not closed
                        if not (
                            hasattr(inventory_fetcher, "_closed") and inventory_fetcher._closed
                        ):
                    ...
                    auth_type = (
                        config.gateway_auth_type
                        if config and config.use_gateway
                        else config.auth_type if config else "unknown"
                    )
                    ...
                    try:
                        if config:
                            if config.use_gateway:
                                if (
                                    config.gateway_auth_type in ["jwt", "google"]
                                    and config.jwt_token
                                ):
                                    # Validate JWT token only if not closed
                                    if not (
                                        hasattr(inventory_fetcher, "_closed")
                                        and inventory_fetcher._closed
                                    ):
                                        jwt_validation = inventory_fetcher.validate_jwt_token()
                                        auth_status = (
                                            "jwt_valid"
                                            if jwt_validation.get("valid")
                                            else "jwt_invalid"
                                        )
                                    else:
                                        auth_status = "jwt_configured"
                                elif (
                                    config.gateway_auth_type == "api_key" and config.gateway_api_key
                                ):
                                    auth_status = "api_key"
                                else:
                                    auth_status = "no_auth"
                                ...
                                # Legacy mode
                                if config.auth_type == "api_token" and config.api_key:
                                    auth_status = "api_token"
                                elif config.auth_type in ["jwt", "bearer"] and config.jwt_token:
                                    auth_status = "jwt_configured"
                                elif config.auth_type == "none" and config.api_key:
                        ...
                        if config:
                            if config.use_gateway:
                                if (
                                    ...
                                    # Don't validate JWT during status check - just check if configured
                                    auth_status = "jwt_configured"
                                elif (
                                    config.gateway_auth_type == "api_key" and config.gateway_api_key
                                ):
